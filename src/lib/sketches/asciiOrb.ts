...
const asciiOrb =
  (theme?: string) =>
  (p: p5): void => {
    const getThemeColor = () => (theme === "light" ? "#8be9fd" : "#bd93f9");
    ...
    p.draw = () => {
      ...
      const light1X =
        Math.cos(time) * lightRadius + Math.cos(time) * lightRadius;
      const light1Y =
        Math.sin(time) * lightRadius + Math.sin(time) * lightRadius;
